...
<?php if ( have_comments() ) : ?>
	...
	<h3 class="comments-title">
		...
		global $post;
		echo '<a href="'.get_post_comments_feed_link( $post->ID ).'" class="rss-feed-link" title="'.esc_attr( __( 'Post comments RSS feed', 'activetab' ) ).'"><i class="ionicon ion-social-rss icon-20"></i></a>';
		?>
	</h3>
...
<?php if ( comments_open() ) : ?>
	...
	<section id="respond" class="respond-form">
		...
		<div id="cancel-comment-reply">
			<p><?php cancel_comment_reply_link( '<span class="btn btn-default"><i class="ionicon ion-ios-close icon-20"></i> '.__( 'Cancel reply', 'activetab' ).'</span>' ); ?></p>
		</div>
		...
		<?php
			...
			$comment_form_args = array(
				'fields' => array(
					'author' => '<div class="form-group comment-form-author"> <label class="control-label" for="author"><strong>'.__( 'Name', 'activetab' ).'</strong></label> <div class="input-group"><span class="input-group-addon"><i class="ionicon ion-ios-person icon-20"></i></span><input id="author" class="form-control" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" ' . $aria_req . ( $req ? ' required ' : '' ) . '></div></div>',
					...
					'url'    => '<div class="form-group comment-form-url"> <label class="control-label" for="url"><strong>'.__( 'Website', 'activetab' ).'</strong> <span class="text-muted smaller-text">'.__( 'Example: http://google.com', 'activetab' ).'</span></label> <div class="input-group"><span class="input-group-addon"><i class="ionicon ion-link icon-20"></i></span><input id="url" class="form-control" name="url" type="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '"></div> </div>',
				),
				'comment_field'        => '<div class="form-group comment-form-comment"> <label class="control-label" for="comment"><strong>'.__( 'Comment', 'activetab' ).'</strong></label> <div class="input-group"><span class="input-group-addon"><i class="ionicon ion-ios-chatbubble icon-20"></i></span><textarea id="comment" class="form-control" name="comment" cols="45" rows="8" required="required"></textarea></div></div>',
